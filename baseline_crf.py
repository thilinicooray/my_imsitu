...
def train_model(max_epoch, eval_frequency, train_loader, dev_loader, traindev_loader, model, encoding, optimizer, save_dir, timing = False):
    model.train()
    ...
    for k in range(0,max_epoch):
        for i, (index, input, target) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                print ("TRAINDev {} average :{:.2f} {} {}".format(total_steps-1, avg_score*100, format_dict(top1_a,"{:.2f}", "1-"), format_dict(top5_a, "{:.2f}", "5-")))
...
if __name__ == "__main__":
    ...
    if args.command == "train":
        ...
        traindev_set = json.load(open(args.dataset_dir +"/train_eval.json"))
        ...
        dataset_traindev = imSituSituation(args.image_dir, traindev_set, encoder, model.dev_preprocess())
        ...
        traindev_loader  = torch.utils.data.DataLoader(dataset_traindev, batch_size = batch_size, shuffle = True, num_workers = 3)

        model.cuda()
        optimizer = optim.Adam(model.parameters(), lr = args.learning_rate , weight_decay = args.weight_decay)
        train_model(args.training_epochs, args.eval_frequency, train_loader, dev_loader, traindev_loader, model, encoder, optimizer, args.output_dir)
